...
interface ConsultationsHistoryProps {
  data: PaginatedResult<ConsultationListRow>;
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Consultas registradas en la clínica</p>
      </div>
          ...
          <div className="space-y-2">
            {data.data.map((consultation) => (
              ...
              >
                ...
                {(consultation.title || consultation.diagnosis) && (
                  <p className="mt-1 text-sm">{consultation.title || consultation.diagnosis}</p>
                )}
              </Link>
            ))}
          </div>
          ...
          {data.totalPages > 1 && (
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                {data.total} consulta{data.total !== 1 ? 's' : ''} · Página {data.page} de{' '}
                {data.totalPages}
              </p>
