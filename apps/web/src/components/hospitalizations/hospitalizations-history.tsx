...
interface HospitalizationsHistoryProps {
  data: PaginatedResult<HospitalizationListRow>;
  initialSearch?: string;
  initialStatus?: string;
}

export function HospitalizationsHistory({
  data,
  initialSearch = '',
  initialStatus = '',
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Internaciones de la clínica</p>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="relative max-w-md flex-1">
          ...
          <Input
            placeholder="Buscar por paciente, motivo, jaula..."
            value={search}
      ...
      {data.data.length === 0 ? (
        <div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">
          No hay internaciones en el historial.
        </div>
      ) : (
        <>
          <div className="space-y-2">
            {data.data.map((stay) => (
              ...
              >
                ...
                <p className="mt-1 text-sm">{stay.reason}</p>
              </Link>
            ))}
          </div>
          ...
          {data.totalPages > 1 && (
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                {data.total} internación{data.total !== 1 ? 'es' : ''} · Página {data.page} de{' '}
                {data.totalPages}
              </p>
